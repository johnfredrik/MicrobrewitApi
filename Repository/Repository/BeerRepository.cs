...
namespace Microbrewit.Repository
{
    public class BeerRepository : GenericDataRepository<Beer>, IBeerRepository
    {
        ...
        public async override Task<int> UpdateAsync(params Beer[] items)
        {
            using (var context = new MicrobrewitContext())
            {
                foreach (var newBeer in items)
                {
                    var dbBeer = context.Beers
                        ...
                        .SingleOrDefault(b => b.Id == newBeer.Id);
                    ...
                    {
                        var dbUserBeer = context.UserBeers.SingleOrDefault(u => u.Username.Equals(newUserBeer.Username) && u.BeerId == newBeer.Id);
                        if (dbUserBeer != null)
                        {
                    ...
                    {
                        var dbBreweryBeer = context.BreweryBeers.SingleOrDefault(b => b.BreweryId == newBreweryBeer.BreweryId && b.BeerId == newBeer.Id);
